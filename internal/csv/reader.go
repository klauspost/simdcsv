...
func (r *Reader) readRecord(dst []string) ([]string, error) {
	...
	if !r.cached {
		...
		if r.Quote > 0 {
			r.cachedQuoteLen = utf8.RuneLen(r.Quote)
			r.cachedEncodedQuote = encodeRune(r.Quote)
			r.cachedQuotes += string(r.Quote)
		}
...
parseField:
	for {
		...
		if len(line) == 0 || r.cachedQuoteLen == 0 || nextRune(line) != r.Quote {
			...
			for {
				...
				if i >= 0 {
					...
					case escape && r.QuoteEscape != r.Quote:
